...
export default class WorkController extends Controller {
  ...
  async createChannels() {
    ...
    try {
      const res = await ctx.model.Work.findOneAndUpdate({id:workId},{
        $push:{
          channels:newChannel
        }
      })
  ...
  @checkPermission({casl:'Channels',mongoose:'Work'},'workNoPermission',{key:'channels.id'})
  async updateChannels(){
    const { ctx, app } = this;
    const { id } = ctx.params;
    const { name } = ctx.request.body;
    try {
      const result = await ctx.model.Work.findOneAndUpdate({'channels.id':id},{
        $set:{
          'channels.$.name':name
        }
      })
  ...
  async delete(){
    const { ctx, service } = this;
    const { id } = ctx.params
    const workData = await ctx.model.Work.findOneAndDelete({id}).select('_id id title').lean()
    ctx.helper.success({ ctx, res: workData })
  }
  ...
  async publish(isTmeplate:boolean){
    const { ctx, service } = this;        
    const url = await this.service.work.publish(parseInt(ctx.params.id),isTmeplate)    
